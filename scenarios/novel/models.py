"""
Novel Generation Scenario - Data Models

Domain-specific models for the novel generation scenario.
These models are migrated from src/api/models.py as part of Phase 1 refactoring.
"""
...
class Novel(Base):
    """Novel entity - root aggregate for novel generation"""
...
class Chapter(Base):
    """Chapter entity - unit of novel generation workflow"""
...
class ChapterDraft(Base):
    """Chapter draft - versioned content storage"""
    ...
    summary = Column(Text, nullable=True)
    critique_data = Column(JSON, nullable=True)  # { "score": int, "comments": str, "actionable_feedback": str }
...
class PendingWrite(Base):
    """
    Pending write approval entity.

    Agent-generated content awaiting user approval before persisting to database.
    Supports human-in-the-loop workflow for quality control.
    """
    ...
    workflow_type = Column(String(64), nullable=True, index=True)  # generate_chapter / outline_only / etc.
...
class NovelSetting(Base):
    """
    Novel global settings - Key-value store.

    Replaces filesystem-based storage for novel configuration.
    Common keys: "bios" (character bios), "world" (world setting), "story_summary", "relation_graph"
    """
    ...
    value = Column(Text, nullable=True)  # JSON-serialized data
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
