...
def main():
    ...
    logger.info(f"开始为小说《{novel_name}》的所有章节生成信息提取、大纲和审阅...")
    ...
    chapters_to_process = sorted(all_chapters)
    logger.info(f"将处理所有章节: {chapters_to_process} (共 {len(chapters_to_process)} 章)")
    ...
    results = [results_dict[ch_num] for ch_num in chapters_to_process]
