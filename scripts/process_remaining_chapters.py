...
def main():
    ...
    logger.info(f"开始为小说《{novel_name}》的后续章节生成信息提取、大纲和审阅...")
    ...
    all_chapters = file_manager.list_chapters(novel_name)
    if not all_chapters:
        logger.error("未找到任何章节")
        return
    
    remaining_chapters = sorted([ch for ch in all_chapters if ch > 10])
    if not remaining_chapters:
        logger.info("没有需要处理的后续章节（第11章及之后）")
        return
    
    logger.info(f"将处理后续章节: {remaining_chapters} (共 {len(remaining_chapters)} 章)")
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        future_to_chapter = {
            ...
            for chapter_num in remaining_chapters
        }
    ...
    results = [results_dict[ch_num] for ch_num in remaining_chapters]
