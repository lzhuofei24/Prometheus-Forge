...
class CharacterChecker(BaseChecker):
    ...
    def check(self, state: AgentState) -> Dict[str, Any]:
        logger.info(f"[CharacterChecker] 开始检查第 {state['chapter_num']} 章的人物表现")
        
        novel_name = state["novel_name"]
        chapter_num = state["chapter_num"]
        draft_content = state.get("draft_content", "")
        outline = state.get("outline", "")
        character_bios = state.get("character_bios", "")
        ...
        prompt_path = project_root / "config" / "prompts" / "character_check.yaml"
        prompt_raw = resolve_prompt("character_check", prompt_path)
        ...
        user_prompt = user_template.format(
            character_bios=character_bios or "（无人物档案）",
        ...
        try:
            ...
            logger.info(f"[CharacterChecker] 评分: {result['score']}")
            return result
        except Exception as e:
            logger.error(f"[CharacterChecker] 解析失败: {e}")
            return {
                "score": 50,
