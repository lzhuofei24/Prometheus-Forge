...
class PlotChecker(BaseChecker):
    ...
    def check(self, state: AgentState) -> Dict[str, Any]:
        logger.info(f"[PlotChecker] 开始检查第 {state['chapter_num']} 章的剧情逻辑")
        ...
        try:
            ...
            logger.info(f"[PlotChecker] 评分: {result['score']}")
            return result
        except Exception as e:
            logger.error(f"[PlotChecker] 解析失败: {e}")
            return {
                "score": 50,
